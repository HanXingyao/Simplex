...
def simplex_solve(c_, a_, b_):
    ...
    while True:
        ...
        # 选择入基变量
        enter_idx = np.argmin(tableau.loc["discs"].tolist()[: -2])
        ...
        tableau.iloc[ratio_min_idx, :] = tableau.iloc[ratio_min_idx, :] / tableau.iloc[ratio_min_idx, enter_idx]
        for i in range(1, tableau.shape[0]):
            if i == ratio_min_idx:
                continue
            tableau.iloc[i, :-1] = tableau.iloc[i, :-1] - tableau.iloc[i, enter_idx] * tableau.iloc[ratio_min_idx, :-1]
